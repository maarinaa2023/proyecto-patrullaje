...
#include "nav_bt/MoveInSection.hpp"
...
geometry_msgs::msg::PoseStamped MoveInSection::generate_random_pose()
{
  ...
  while (attempts < max_attempts)
  {
    ...
    if (map_image_.at<uchar>(py, px) > 200) {
      ...
      RCLCPP_INFO(node_->get_logger(), "[MoveInSection] Punto válido encontrado.");
      return pose;
    }
  }

  RCLCPP_WARN(node_->get_logger(), "[MoveInSection] No se encontró un punto válido. Usando default.");
  pose.header.frame_id = "map";
...
{
  factory.registerNodeType<nav_bt::MoveInSection>("MoveInSection");
}
